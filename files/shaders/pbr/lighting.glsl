...
void perLightSun(out vec3 diffuseOut, vec3 viewPos, vec3 viewNormal)
{
    ...
    diffuseOut = SRGBToLinearApprox(lcalcDiffuse(0).xyz) * lambert;
}
...
// cull non-FFP point lighting by radius, light is guaranteed to not fall outside this bound with our cutoff
    ...
    ambientOut = SRGBToLinearApprox(lcalcAmbient(lightIndex)) * illumination;
    ...
    diffuseOut = SRGBToLinearApprox(lcalcDiffuse(lightIndex)) * lambert;
}
...
#endif
{
    ...
    ambientLight = SRGBToLinearApprox(gl_LightModel.ambient.xyz);
#if PER_PIXEL_LIGHTING
