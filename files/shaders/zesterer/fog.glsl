...
#endif
#if @exponentialFog
    highp float fogValue = 1.0 - exp(-2.0 * max(0.0, dist - gl_Fog.start/2.0) / (gl_Fog.end - gl_Fog.start/2.0));
#else
    highp float fogValue = clamp((dist - gl_Fog.start) * gl_Fog.scale, 0.0, 1.0);
#endif
    color.xyz = mix(color.xyz, gl_Fog.color.xyz, fogValue);

#if @skyBlending && !@useOVR_multiview
    highp float fadeValue = clamp((far - dist) / (far - skyBlendingStart), 0.0, 1.0);
