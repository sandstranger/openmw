...
void main(void)
{
	...
	*/
	...
	waterColor = GetWaterColor( realWaterDepthfix/dmeter, depthrealfix/dmeter, refraction, lcalcDiffuse(0) + gl_LightModel.ambient.xyz);
   ...
   gl_FragData[0].xyz = mix( a, b ,  fresnel) + specular * lcalcDiffuse(0) * lcalcDiffuse(0) + clamp(dist/6000.0,0.0,0.2) * clamp(1.0 * vec3(rainRipple.w * (lcalcDiffuse(0) + gl_LightModel.ambient.xyz)), 0.0, 1.0);
    ...
    gl_FragData[0].w = clamp(fresnel*6.0 + specular * lcalcSpecular(0).w, 0.0, 1.0);     //clamp(fresnel*2.0 + specular * gl_LightSource[0].specular.w, 0.0, 1.0);
#endif
